...
local SafeZoneTitle = Config.safezoneTitle
local SafeZoneMessage = Config.safezoneTitle
local Time = Config.time
local InSafeType = Config.InSafetype
local OutOfSafeTitle = Config.outofsafeTitle
local OutOfSafeMessage = Config.outofsafeMessage
local OutOfSafeType = Config.OutOfSafetype

    if Time > 15000 or Time < 1000 then
      Time = 5000
	  print("Time set over/under limit in config, notify server developers set to default 5 second popup")
    end 
Citizen.CreateThread(function()
	while true do
		...
		if dist <= Config.radius then
			if not SafezoneIn then
                ...
                exports['okokNotify']:Alert(SafeZoneTitle, SafeZoneMessage, Time, InSafeType)
				end
				SafezoneIn = true
				SafezoneOut = false
			end
			...
			if not SafezoneOut then
				...
				if Config.showNotification then
                exports['okokNotify']:Alert(OutOfSafeTitle, OutOfSafeMessage, Time, OutOfSafeType)
				end
